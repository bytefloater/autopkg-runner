...
class TestListRunsView:
    ...
    def test_no_filters_returns_all_runs(self, api_client, run):
    ...
    def test_start_date_filters_older_runs(self, api_client):
        ...
        resp = api_client.get(self.url, {'start_date': '2024-01-01'})
        assert resp.status_code == 200
        data = resp.json()
        ids = [r['id'] for r in data]
    ...
    def test_end_date_filters_newer_runs(self, api_client):
        old_run = self._create_run_with_started_at(
            datetime(2020, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        )
        recent_run = self._create_run_with_started_at(
        ...
        resp = api_client.get(self.url, {'end_date': '2021-01-01'})
        assert resp.status_code == 200
        data = resp.json()
        ids = [r['id'] for r in data]
    ...
    def test_invalid_start_date_returns_400(self, api_client):
        resp = api_client.get(self.url, {'start_date': 'bad-date'})
        assert resp.status_code == 400
        data = resp.json()
        assert 'error' in data

    def test_invalid_end_date_returns_400(self, api_client):
        resp = api_client.get(self.url, {'end_date': 'not-a-date'})
        assert resp.status_code == 400
        data = resp.json()
        assert 'error' in data

    def test_run_fields_present(self, api_client, run):
        resp = api_client.get(self.url)
        assert resp.status_code == 200
        data = resp.json()
...
class TestGetRunDataView:
    ...
    def test_valid_run_returns_200_with_nested_data(self, api_client, run):
        resp = api_client.get(self.url, {'uuid': str(run.id)})
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_run_with_stage_execution_included(self, api_client, run):
    ...
    def test_run_with_log_entries_included(self, api_client, run):
    ...
    def test_run_with_recipe_results_included(self, api_client, run):
